...
nr_rep      = 1                          # Number of Monte Carlo repetitions 
...
p6 = plt.figure()
ax = p6.add_subplot(111, projection='3d')
t_3d, f_3d = np.meshgrid( np.linspace(0,(h_OFDM.shape[-1])*1/F/2,h_OFDM.shape[-1]), np.linspace(0,(L-1)*F,L))
ax.plot_surface(t_3d/1e-3, f_3d/1e6, (np.abs(h_OFDM)), cmap=cm.coolwarm)
ax.set_xlabel('Time [ms]')
ax.set_ylabel('Frequency [MHz]')
ax.set_zlabel('Transfer Function, |H(t,f)|')
ax.set_title('Possible Channel Realization')
